...
from app.api.deps import require_admin, require_admin_or_front_desk
...
async def create_class_session(
    *,
    ...
    schedule = await crud.class_schedule.get(db, id=session_in.class_schedule_id)
    if not schedule:
...
async def read_class_sessions(
    *,
    ...
    if date_from:
        stmt = stmt.where(ClassSession.starts_at >= datetime.combine(date_from, datetime.min.time()))

    if date_to:
        stmt = stmt.where(ClassSession.starts_at <= datetime.combine(date_to, datetime.max.time()))
